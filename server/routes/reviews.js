...
router.post("/:restaurantId", auth, async (req, res) => {
  try {
    ...
    if (existingReview) {
      return res.status(400).json({ success: false, msg: "You have already reviewed this restaurant" });
    }
    ...
    res.status(500).json({ success: false, msg: "Server error while adding review" });
  }
});
...
router.get("/:restaurantId", async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, msg: "Server error while fetching reviews" });
  }
});
...
router.delete("/:id", auth, async (req, res) => {
  try {
    ...
    // Use findByIdAndDelete to trigger Review post('findOneAndDelete') hook
    await Review.findByIdAndDelete(id);

    res.status(200).json({ success: true, msg: "Review deleted successfully" });
  } catch (err) {
    console.error("❌ Error deleting review:", err.message);
    res.status(500).json({ success: false, msg: "Server error while deleting review" });
  }
});
